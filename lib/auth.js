...
/**
 * Hashes a 6-digit OTP with bcrypt before it is persisted to the database,
 * so a DB read/breach never exposes the OTP in plaintext.
 */
...
/**
 * Compares a submitted OTP against its stored bcrypt hash.
 * Returns false (never throws) on any malformed input.
 */
export async function compareOtp(inputOtp, storedHash) {
  const a = typeof inputOtp   === 'string' ? inputOtp   : '';
  const h = typeof storedHash === 'string' ? storedHash : '';
  if (!/^\d{6}$/.test(a) || !h.startsWith('$2')) return false;
  try {
    return await bcrypt.compare(a, h);
  } catch {
    return false;
  }
}
