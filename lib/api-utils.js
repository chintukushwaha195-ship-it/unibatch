 ...
 * Always sets Cache-Control: no-store — every route in this app reads
 * live DB state, so a cached response (browser, CDN, or Next's Route
 * Handler cache) would silently show stale data after an admin edit.
 *
 * @param {unknown} data   — Serialisable body.
 * @param {number}  status — HTTP status code (default 200).
 */
export function json(data, status = 200) {
  const res = applyCors(NextResponse.json(data, { status }));
