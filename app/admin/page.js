...
import {
  LogOut, RefreshCw, Check, EyeOff, Star, StarOff, ArrowLeft,
...
export default function AdminDashboard() {
  ...
  const forceSync = async () => {
    setSyncing(true);
    try {
      const res = await authFetch('/api/admin/sync', { method: 'POST' });
      const data = await res.json();
      if (data.ok) toast.success(`Sync ok · inserted ${data.inserted} · scanned ${data.scanned} blocks`);
      ...
      <header className="border-b border-white/10 backdrop-blur-lg sticky top-0 z-40 bg-navy-950/60">
        <div className="container mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button onClick={forceSync} disabled={syncing} variant="outline" size="sm" className="rounded-full border-white/15 bg-white/5 hover:bg-white/10">
              {syncing ? <Loader2 className="w-4 h-4 animate-spin" /> : <><RefreshCw className="w-4 h-4 mr-1" /> Sync now</>}
            </Button>
